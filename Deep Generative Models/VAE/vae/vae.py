...
class VAE(nn.Module):
    ...
    def kl_divergence(self, mu: TensorType['batch_size', 'latent_dim'], logsigma: TensorType['batch_size', 'latent_dim']) -> TensorType['batch_size']:
        ...
        # YOUR CODE HERE
        sigma = torch.exp(logsigma)
        kl = -0.5 * torch.sum(1 + 2 * logsigma - mu**2 - sigma**2, dim=1)
    ...
    def elbo(self, x: TensorType['batch_size', 'input_dim']) -> TensorType['batch_size']:
        ...
        z = self.sample_with_reparametrization(mu, log_sigma)
        
        # Decode
        theta = self.decoder(z)
        
        # Compute log likelihood
        recon_loss = nn.functional.binary_cross_entropy(theta, x, reduction='none').sum(dim=1)
    ...
    def sample(self, num_samples: int, device: str='cpu') -> Tuple[
        ...
        # Decode sampled latent codes to get parameters of the output distribution
        theta = self.decoder(z)
        
        # Generate samples from the output distribution (Bernoulli for binary data)
